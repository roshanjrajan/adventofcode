...
class FS:
    ...
    def get_part2(self, needed_size):
        # breakpoint()
        if self.size < needed_size or self.type == 1:
            return float('inf')

        min_size = self.size
        for _, child in self.children.items():
            min_size = min(min_size, child.get_part2(needed_size))
...
def compute(s: str) -> int:
    ...
    needed_size = 30000000 - (70000000 - rootNode.size)
    print(needed_size)
    return rootNode.get_part2(needed_size)
